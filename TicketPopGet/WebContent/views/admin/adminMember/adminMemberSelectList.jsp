...
<%
	...
	String selectUser = String.valueOf(request.getAttribute("selectUser"));
	String selectUserType = String.valueOf(request.getAttribute("selectUserType"));
%>
<!DOCTYPE html>
<html>
<head>
...
<body>
    ...
    <br><br>
		...
		<%if(p.getCurrentPage() != p.getStartPage()) {%>
        	<button onclick="location.href='<%=request.getContextPath()%>/selectMember.adme?currentPage=1&selectUser=<%=selectUser%>&selectUserType=<%=selectUserType%>'">&lt;&lt;</button>
       	 	<button onclick="location.href='<%=request.getContextPath()%>/selectMember.adme?currentPage=<%=p.getCurrentPage() - 1%>&selectUser=<%=selectUser%>&selectUserType=<%=selectUserType%>'">&lt;</button>
        <%} %>
        <%for(int i=p.getStartPage(); i<=p.getEndPage(); i++) {%>
        	<%if(i != p.getCurrentPage()){ %>
        		<button onclick="location.href='<%=request.getContextPath()%>/selectMember.adme?currentPage=<%=i%>&selectUser=<%=selectUser%>&selectUserType=<%=selectUserType%>'"><%=i%></button>
        	<%}else{ %>
       ...
       <%} %>
        <%if(p.getCurrentPage() != p.getEndPage()) { %>
        	<button onclick="location.href='<%=request.getContextPath()%>/selectMember.adme?currentPage=<%=p.getCurrentPage() + 1%>&selectUser=<%=selectUser%>&selectUserType=<%=selectUserType%>'">&gt;</button>
        	<button onclick="location.href='<%=request.getContextPath()%>/selectMember.adme?currentPage=<%=p.getMaxPage()%>&selectUser=<%=selectUser%>&selectUserType=<%=selectUserType%>'">&gt;&gt;</button>   
        <%} %>
    	</div>
    </div>
      <!-- The Modal -->
