...
<body>
    ...
    <form action="<%=request.getContextPath()%>/searchList.adno" style="float: right;" method="post">
    ...
    <div align="center">
		...
		<%if(p.getCurrentPage() != p.getStartPage()) {%>
        	<button onclick="location.href='<%=request.getContextPath()%>/searchList.adno?currentPage=1&selectKeyword=<%=key%>'">&lt;&lt;</button>
       	 	<button onclick="location.href='<%=request.getContextPath()%>/searchList.adno?currentPage=<%=p.getCurrentPage() - 1%>&selectKeyword=<%=key%>'">&lt;</button>
        <%} %>
        <%for(int i=p.getStartPage(); i<=p.getEndPage(); i++) {%>
        	<%if(i != p.getCurrentPage()){ %>
        		<button onclick="location.href='<%=request.getContextPath()%>/searchList.adno?currentPage=<%=i%>&selectKeyword=<%=key%>'"><%=i%></button>
        	<%}else{ %>
       ...
       <%} %>
        <%if(p.getCurrentPage() != p.getEndPage()) { %>
        	<button onclick="location.href='<%=request.getContextPath()%>/searchList.adno?currentPage=<%=p.getCurrentPage() + 1%>&selectKeyword=<%=key%>'">&gt;</button>
        	<button onclick="location.href='<%=request.getContextPath()%>/searchList.adno?currentPage=<%=p.getMaxPage()%>&selectKeyword=<%=key%>'">&gt;&gt;</button>   
        <%} %>  
    </div>
