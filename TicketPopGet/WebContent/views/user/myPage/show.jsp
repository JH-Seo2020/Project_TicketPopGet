	...
	<%@ include file="/views/common/menubar.jsp"%>
            ...
            <table class="table table-hover" style="width: 950px; height: 70px; margin-left: 30px; text-align: center;">
              ...
              <tbody>
	                ...
	                <tr>
	                  ...
	                  <td><button onclick="location.href='<%=contextPath%>/review_write.my?tno=<%=ps.getTicketNo()%>
	                                &userNo=<%=loginUser.getUserNo()%>&cno=<%=ps.getContentNo()%>'">후기작성</button><td>
	                </tr>
	                <%} else {%>
	                  <tr>
	                  <td><%=ps.getTicketNo() %></td>
		...
		<%if(content == null){ %>
	        <div class="pagination" align="center" style="margin-top: 60px; margin-left: 50%;">
	     		<%if(currentPage != 1){ %>
	     			<!-- 맨 처음으로(<<) -->
	     			<a href="<%=contextPath%>/show.my?currentPage=1&userNo=<%=loginUser.getUserNo()%>" class=" btn-prev"><i class="fa fa-chevron-circle-left"></i>Prev</a>
	            <%} %>
	            
	            <%for(int p=startPage; p<=endPage; p++){ %>
	            	<%if(p!=currentPage){ %>
		           		<a href="<%=contextPath%>/show.my?currentPage=<%=p%>&userNo=<%=loginUser.getUserNo()%>"><span><%=p %></span></a>
		            <%}else { %>
	            ...
	            <%if(currentPage != maxPage){ %>
	            	<!-- 맨 끝으로(>>) -->
	            	<a href="<%=contextPath%>/show.my?currentPage=<%=maxPage%>&userNo=<%=loginUser.getUserNo()%>" class=" btn-next"><i class="fa fa-chevron-circle-right"></i>NEXT</a>
	            <%} %>
	        </div>
	     <%} else{ %>
	        <div class="pagination" align="center" style="margin-top: 60px; margin-left: 50%;">
		     		<%if(currentPage != 1){ %>
		     			<!-- 맨 처음으로(<<) -->
		     			<a href="<%=contextPath%>/show_content.my?currentPage=1&userNo=<%=loginUser.getUserNo()%>&content=<%=content%>" class=" btn-prev"><i class="fa fa-chevron-circle-left"></i>Prev</a>
		            <%} %>
		            
		            <%for(int p=startPage; p<=endPage; p++){ %>
		            	<%if(p!=currentPage){ %>
			           		<a href="<%=contextPath%>/show_content.my?currentPage=<%=p%>&userNo=<%=loginUser.getUserNo()%>&content=<%=content%>"><span><%=p %></span></a>
			            <%}else { %>
			            	<a href="javascript:void(0);"><span><%=p %></span></a>
			            <%} %>
		            <%} %>
		            ...
		            <%if(currentPage != maxPage){ %>
		            	<!-- 맨 끝으로(>>) -->
		            	<a href="<%=contextPath%>/show_content.my?currentPage=<%=maxPage%>&userNo=<%=loginUser.getUserNo()%>&content=<%=content%>" class=" btn-next"><i class="fa fa-chevron-circle-right">&gt;&gt;</i>Next</a>
		            <%} %>
		     </div>
        <%} %>
